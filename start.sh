...
if [ "$CURRENT" == "d8cbae94787a" ]; then
    ...
    echo "🔍 Checking if newer tables already exist..."
...
PYEOF
        ...
        if [ "$TEAMS_EXISTS" == "true" ]; then
            echo "✓ Found teams table, stamping to head (h4i5j6k7l8m9)..."
            alembic stamp head
        else
        ...
        echo "🔄 Applying new migrations (f2b3c4d5e6f7, g3h4i5j6k7l8, h4i5j6k7l8m9)..."
        alembic upgrade head
    fi
elif [ -z "$CURRENT" ]; then
